...
const platformPlans = [
  ...
  { name: "Plano Completo", tone: "gold", featured: true, intro: "Tudo do Plano Estratégico, com organização total.", features: ["Cronograma personalizado", "Plano de estudos", "Organização automática", "Ajuste do cronograma", "Controle da evolução"], options: [{ price: "R$ 79,90", access: "2 meses de acesso" }, { price: "R$ 99,90", access: "3 meses de acesso" }] },
];
...
export default function Home() {
  ...
  return <main>
    ...
    <section className="section platform-tour"><div className="container"><div className="section-heading"><span className="eyebrow">VEJA A PLATAFORMA POR DENTRO</span><h2>RECURSOS REAIS PARA ORGANIZAR TODA A SUA PREPARAÇÃO.</h2></div>
      ...
      <div className="tour-row reverse"><div className="tour-copy"><span className="eyebrow">📚 QUESTÕES DIRECIONADAS</span><h2>MAIS DE 5.000 QUESTÕES PARA OS PRÓXIMOS CONCURSOS.</h2><p>O banco reúne mais de 5.000 questões inéditas e também questões das bancas dos próximos concursos. O aluno navega pela estrutura do edital e monta baterias personalizadas por disciplina, assunto e subassunto.</p><ul><li>✓ Mais de 5.000 questões inéditas</li><li>✓ Questões das bancas dos próximos concursos</li><li>✓ Filtros inteligentes por edital, disciplina e assunto</li></ul></div><BrowserShot src={screens.questoes} alt="Banco de questões real da plataforma"/></div>
    ...
    <section className="section plans-section" id="planos"><div className="container"><div className="section-heading"><span className="eyebrow">ESCOLHA O PLANO IDEAL</span><h2>COMECE NO NÍVEL DE DIREÇÃO QUE VOCÊ PRECISA.</h2><p>Todos os valores são de pagamento único.</p></div><div className="plans-grid">{platformPlans.map(plan=><article className={`plan ${plan.featured?"featured":""} ${plan.tone}`} key={plan.name}>{plan.featured&&<span className="badge">🔥 MAIS ESCOLHIDO</span>}<h3>{plan.name}</h3>{plan.options ? <div style={{display:"grid",gap:"12px",margin:"18px 0 22px"}}>{plan.options.map(option=><div key={option.access} style={{border:"1px solid rgba(255,190,32,.55)",borderRadius:"16px",padding:"16px",background:"rgba(255,190,32,.06)"}}><strong style={{display:"block",marginBottom:"4px"}}>{option.price}</strong><small style={{display:"block",marginBottom:"14px"}}>{option.access}</small><a href={whatsappLink(`Olá! Tenho interesse no ${plan.name}, no valor de ${option.price}, com ${option.access}.`)} target="_blank" rel="noreferrer">ESCOLHER {option.access.toUpperCase()}</a></div>)}</div> : <><strong>{plan.price}</strong><small>{plan.access}</small></>}<p>{plan.intro}</p><ul>{plan.features.map(f=><li key={f}>✓ {f}</li>)}</ul>{!plan.options&&<a href={whatsappLink(`Olá! Tenho interesse no ${plan.name}, no valor de ${plan.price}.`)} target="_blank" rel="noreferrer">QUERO ESTE PLANO</a>}</article>)}</div>
      <article className="premium-plan"><div><span className="eyebrow">👑 PLANO MENTORIA PREMIUM</span><h2>A PREPARAÇÃO MAIS COMPLETA DA LC CONCURSOS.</h2><p>Todos os recursos da plataforma, com mentoria individual, encontro semanal, ajustes no cronograma, análise personalizada e direcionamento até a prova.</p><ul><li>✓ Mentoria individual</li><li>✓ Encontro semanal</li><li>✓ Ajustes no cronograma</li><li>✓ Análise personalizada</li><li>✓ Direcionamento até a prova</li></ul></div><div className="premium-options">{premiumPlans.map(([period,price,label])=><div key={period}><span>{period}</span><strong>{price}</strong>{label&&<small>{label}</small>}<a className="premium-choice" href={whatsappLink(`Olá! Escolhi o Plano Mentoria Premium de ${period.replace(/^[^\s]+\s/, "")}, no valor de ${price}. Gostaria de concluir minha matrícula.`)} target="_blank" rel="noreferrer">ESCOLHER ESTE PLANO</a></div>)}</div></article>
    </div></section>
